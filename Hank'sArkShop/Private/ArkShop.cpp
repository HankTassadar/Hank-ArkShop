		...
		ArkShop::TimedRewards::Get().AddTask(
			FString::Format("Points_{}", steam_id), steam_id, [steam_id]()
			{
				...
				for (auto group_iter = groups_map.begin(); group_iter != groups_map.end(); ++group_iter)
				{
					...
					if (ArkShop::database->IsPlayerInGroup(steam_id, group_name))
					{
...
void ReadConfig()
{
	const std::string config_path = ArkApi::Tools::GetCurrentDir() + "/ArkApi/Plugins/Hank'sArkShop/config.json";
...
void Unload()
{
	...
	ArkApi::GetCommands().RemoveOnTimerCallback("OnlineTime");
